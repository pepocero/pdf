class PDFEditor {
    ...
    async loadPDF(file) {
        ...
        this.currentFileName = file.name; // Guardar nombre para persistencia local
        ...
        try {
            ...
            // Cargar ediciones previas de LocalStorage
            this.loadFromLocalStorage(file.name);
        } catch (error) {
    ...
    setTool(tool) {
        ...
        if (this.selectedShape && tool !== 'select' && tool !== 'resize') {
            this.deselectShape();
        }
        
        // Si se cambia de herramienta y no es 'resize', limpiar manijas de redimensionado
        if (tool !== 'resize') {
            document.querySelectorAll('.resize-handle-external').forEach(h => h.remove());
        }
        
        // Si se cambia a 'resize' y hay un elemento seleccionado, activarlo
        if (tool === 'resize' && this.selectedElement) {
            this.makeResizable(this.selectedElement);
        }
    ...
    makeDraggable(element) {
        ...
        element.addEventListener('mousedown', (e) => {
            // Impedir que el navegador gestione drag and drop nativo
            if (element.tagName === 'IMG') e.preventDefault();
            ...
            // Si la herramienta es 'eraser' y es un dibujo, no arrastrar aquí (se borra en otro listener)
            if (this.currentTool === 'eraser' && isSVG) return;
            ...
            // Solo permitir arrastrar con la herramienta de selección o mano
            if (this.currentTool !== 'select' && this.currentTool !== 'hand') return;

            e.preventDefault();
            e.stopPropagation(); // Evitar que el canvas detecte mousedown
            ...
            if (isSVG) {
                initialX = parseFloat(element.getAttribute('data-tx')) || 0;
                initialY = parseFloat(element.getAttribute('data-ty')) || 0;
            } else {
        ...
        document.addEventListener('mousemove', (e) => {
            if (isDragging) {
                ...
                if (isSVG) {
                    const nx = initialX + deltaX;
                    const ny = initialY + deltaY;
                    element.setAttribute('transform', `translate(${nx}, ${ny})`);
                    element.setAttribute('data-tx', nx);
                    element.setAttribute('data-ty', ny);
                } else {
                    element.style.left = (initialX + deltaX * this.zoom) + 'px';
                    element.style.top = (initialY + deltaY * this.zoom) + 'px';
                }
            }
        });
        ...
        document.addEventListener('mouseup', () => {
            if (isDragging) {
                isDragging = false;
                element.style.cursor = isSVG ? 'move' : 'move';
                element.classList.remove('dragging');
                
                // Guardar cambios en el almacenamiento local
                this.saveToLocalStorage();
                
                // Verificar si el elemento cambió de página (solo para elementos HTML)
                if (!isSVG) {
        ...
        // Manejar eventos táctiles (optimizado)
        element.addEventListener('touchstart', (e) => {
            if (element.contentEditable === 'true' || element.tagName === 'TEXTAREA') return;
            if (this.currentTool === 'eraser' && isSVG) return;
        ...
        document.addEventListener('touchmove', (e) => {
            if (isDragging) {
                e.preventDefault();
                const touch = e.touches[0];
                const deltaX = (touch.clientX - startX) / this.zoom;
                const deltaY = (touch.clientY - startY) / this.zoom;
                
                if (isSVG) {
                    const nx = initialX + deltaX;
                    const ny = initialY + deltaY;
                    const nsx = parseFloat(element.getAttribute('data-sx')) || 1;
                    const nsy = parseFloat(element.getAttribute('data-sy')) || 1;
                    element.setAttribute('transform', `translate(${nx}, ${ny}) scale(${nsx}, ${nsy})`);
                    element.setAttribute('data-tx', nx);
                    element.setAttribute('data-ty', ny);
                } else {
                    element.style.left = (initialX + deltaX * this.zoom) + 'px';
        ...
        document.addEventListener('touchend', () => {
            if (isDragging) {
                ...
                element.classList.remove('dragging');
                this.saveToLocalStorage();
                if (!isSVG) this.checkAndUpdateElementPage(element);
            }
        });
    ...
    makeResizable(element) {
        ...
        const parent = isSVG ? element.ownerSVGElement.parentElement : element.parentElement;
        if (!parent) return;
        
        const resizeHandle = document.createElement('div');
        resizeHandle.className = 'resize-handle-external';
        resizeHandle.dataset.targetId = element.id || 'el-' + Date.now();
        if (!element.id) element.id = resizeHandle.dataset.targetId;
        ...
        let startSX, startSY, startTX, startTY;
        let bbox = null;
        ...
        resizeHandle.addEventListener('mousedown', (e) => {
            ...
            if (isSVG) {
                ...
                startSX = parseFloat(element.getAttribute('data-sx')) || 1;
                startSY = parseFloat(element.getAttribute('data-sy')) || 1;
                startTX = parseFloat(element.getAttribute('data-tx')) || 0;
                startTY = parseFloat(element.getAttribute('data-ty')) || 0;
                
                // Usar dimensiones reales proyectadas para el cálculo inicial
                startWidth = bbox.width * startSX;
                startHeight = bbox.height * startSY;
            } else {
                startWidth = parseInt(element.style.width) || (element.offsetWidth / this.zoom);
                startHeight = parseInt(element.style.height) || (element.offsetHeight / this.zoom);
            }
        ...
        const handleMouseMove = (e) => {
            if (isResizing) {
                ...
                let newWidth = Math.max(10, startWidth + deltaX / this.zoom);
                let newHeight = Math.max(10, startHeight + deltaY / this.zoom);
                
                if (isSVG) {
                    const nsx = newWidth / bbox.width;
                    const nsy = newHeight / bbox.height;
                    
                    // CORRECCIÓN DE PIVOTE: Ajustar tx/ty para que la esquina superior izquierda (bbox.x, bbox.y) se mantenga fija
                    // tx_new = start_tx + bbox.x * (start_sx - nsx)
                    const ntx = startTX + bbox.x * (startSX - nsx);
                    const nty = startTY + bbox.y * (startSY - nsy);
                    
                    element.setAttribute('transform', `translate(${ntx}, ${nty}) scale(${nsx}, ${nsy})`);
                    element.setAttribute('data-tx', ntx);
                    element.setAttribute('data-ty', nty);
                    element.setAttribute('data-sx', nsx);
                    element.setAttribute('data-sy', nsy);
                } else {
                    ...
                    element.style.width = (newWidth * this.zoom) + 'px';
                    element.style.height = (newHeight * this.zoom) + 'px';
                }
        ...
        const handleMouseUp = () => {
            if (isResizing) {
                ...
                this.saveToLocalStorage();
                this.showNotification('Elemento redimensionado', 'success');
            }
        };
    ...
    updateResizeHandlePosition(element, resizeHandle) {
        ...
        if (isSVG) {
            ...
            const tx = parseFloat(element.getAttribute('data-tx')) || 0;
            const ty = parseFloat(element.getAttribute('data-ty')) || 0;
            const sx = parseFloat(element.getAttribute('data-sx')) || 1;
            const sy = parseFloat(element.getAttribute('data-sy')) || 1;
            
            // Posicionar en la esquina inferior derecha del BBox escalado y trasladado
            handleLeft = (tx + (bbox.x + bbox.width) * sx) * this.zoom;
            handleTop = (ty + (bbox.y + bbox.height) * sy) * this.zoom;
        } else {
            const elementLeft = parseInt(element.style.left) || 0;
            const elementTop = parseInt(element.style.top) || 0;
            const elementWidth = element.offsetWidth;
            const elementHeight = element.offsetHeight;
            handleLeft = elementLeft + elementWidth - 10;
            handleTop = elementTop + elementHeight - 10;
        }
    ...
    // --- Persistencia Local (Privacidad) ---

    saveToLocalStorage() {
        if (!this.currentFileName) return;

        // Antes de guardar, actualizar los offsets (tx, ty) y escalas (sx, sy) de todos los dibujos actuales
        this.drawings.forEach(d => {
            ...
            if (el) {
                ...
                const sy = parseFloat(el.getAttribute('data-sy')) || 1;
                d.tx = tx;
                d.ty = ty;
                d.sx = sx;
                d.sy = sy;
        ...
        localStorage.setItem(`pdf_edits_${this.currentFileName}`, JSON.stringify(data));
        console.log('Cambios guardados localmente para:', this.currentFileName);
    }

    loadFromLocalStorage(fileName) {
        const savedData = localStorage.getItem(`pdf_edits_${fileName}`);
        if (savedData) {
            try {
                ...
                this.showNotification('Restauradas ediciones locales guardadas', 'info');
                
                // Restaurar visualmente tras un breve delay para asegurar que el DOM esté listo
                setTimeout(() => {
                    this.restoreAllDrawings();
                    this.restoreAllHighlights();
                }, 800);
            } catch (e) {
                console.error('Error al cargar del almacenamiento local:', e);
            }
        }
    }

    clearLocalStorage(fileName) {
        localStorage.removeItem(`pdf_edits_${fileName}`);
    }
    ...
    clearSelections() {
        ...
        // Remover clases de selección de elementos HTML y SVG
        document.querySelectorAll('.selected, .selected-svg').forEach(el => {
            el.classList.remove('selected');
            el.classList.remove('selected-svg');
    ...
    selectElement(element) {
        ...
        if (element instanceof SVGElement) {
            element.classList.add('selected-svg');
        } else {
            element.classList.add('selected');
        }
        
        // Si la herramienta actual es 'resize', activar manijas inmediatamente
    ...
    saveDrawingState(pageNum) {
        if (this.currentDrawing && this.currentDrawing.length > 0) {
            ...
            if (overlay) {
                ...
                g.id = `drawing-group-${pageNum}-${Date.now()}`;
                ...
                path.style.stroke = document.getElementById('strokeColor')?.value || '#000000';
                path.style.strokeWidth = document.getElementById('strokeWidth')?.value || '2';
                ...
                clickPath.style.strokeWidth = '15';
                clickPath.style.fill = 'none';
                clickPath.style.pointerEvents = 'auto';
                clickPath.style.cursor = 'move';
                
                // Agregar listeners al grupo o al clickPath
                clickPath.addEventListener('mousedown', (e) => {
                    if (this.currentTool === 'eraser') {
                        e.stopPropagation();
                        this.removeDrawing(g.id);
                    } else if (this.currentTool === 'select') {
                        this.selectElement(g);
                    }
                });
                
                // Agregar ambos paths al grupo
                g.appendChild(path);
                g.appendChild(clickPath);
                
                // Hacer el grupo arrastrable con nuestra lógica unificada
                this.makeDraggable(g);
                
                // Agregar grupo al contenedor
                svgContainer.appendChild(g);
                
                // Inicializar atributos en el elemento para persistencia y redimensionado
                g.setAttribute('data-tx', '0');
                g.setAttribute('data-ty', '0');
                g.setAttribute('data-sx', '1');
                g.setAttribute('data-sy', '1');
                ...
                this.saveToLocalStorage(); // Guardar localmente
    ...
    createShapeElement(shapeData) {
        ...
        const g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
        const shapeId = `shape-${shapeData.type}-${shapeData.pageNum}-${Date.now()}`;
        g.id = shapeId;
        ...
        if (shapeElement && drawingData) {
            shapeElement.id = `inner-${drawingData.id}`;
            ...
            // Agregar forma al grupo
            g.appendChild(shapeElement);
            
            // Crear el clickPath para facilitar la selección de líneas
            if (shapeData.type === 'line') {
                const clickPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                const d = `M ${shapeData.startX} ${shapeData.startY} L ${shapeData.endX} ${shapeData.endY}`;
                ...
                // Listener para la propia forma (rect, circle)
            ...
            // Inicializar transform data para persistencia
            drawingData.id = g.id;
            drawingData.tx = 0;
            drawingData.ty = 0;
            drawingData.sx = 1;
            ...
            // Establecer atributos iniciales en el elemento
            g.setAttribute('data-tx', '0');
            g.setAttribute('data-ty', '0');
            g.setAttribute('data-sx', '1');
            g.setAttribute('data-sy', '1');
            ...
            console.log('Forma guardada y hecha interactiva:', drawingData.id);
        }
    }
        ...
        this.drawings.forEach(drawingData => {
            ...
            if (overlay) {
                ...
                if (!document.getElementById(drawingData.id)) {
                    ...
                    // Crear el grupo wrapper
                    const g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
                    g.id = drawingData.id;
                    
                    // Aplicar transformaciones guardadas
                    if (drawingData.tx !== undefined && drawingData.ty !== undefined) {
                        const sx = drawingData.sx || 1;
                        const sy = drawingData.sy || 1;
                        g.setAttribute('transform', `translate(${drawingData.tx}, ${drawingData.ty}) scale(${sx}, ${sy})`);
                        g.setAttribute('data-tx', drawingData.tx);
                        g.setAttribute('data-ty', drawingData.ty);
                        g.setAttribute('data-sx', sx);
                        g.setAttribute('data-sy', sy);
                    }
                    ...
                    if (drawingData.type === 'path') {
                        ...
                        shapeElement.setAttribute('d', drawingData.pathData);
                        shapeElement.style.stroke = drawingData.stroke;
                        shapeElement.style.strokeWidth = drawingData.strokeWidth;
                        shapeElement.style.fill = drawingData.fill || 'none';
                        
                        clickElement = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                        clickElement.setAttribute('d', drawingData.pathData);
                        clickElement.style.stroke = 'transparent';
                        clickElement.style.strokeWidth = '15';
                        clickElement.style.fill = 'none';
                        ...
                        clickElement = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                        const d = `M ${drawingData.x1} ${drawingData.y1} L ${drawingData.x2} ${drawingData.y2}`;
                        clickElement.setAttribute('d', d);
                        clickElement.style.stroke = 'transparent';
                        clickElement.style.strokeWidth = '15';
                        clickElement.style.fill = 'none';
                        ...
                        shapeElement.setAttribute('width', drawingData.width);
                        shapeElement.setAttribute('height', drawingData.height);
                        shapeElement.style.stroke = drawingData.stroke;
                        shapeElement.style.strokeWidth = drawingData.strokeWidth;
                        shapeElement.style.fill = drawingData.fill || 'none';
                        clickElement = shapeElement;
                        
                    } else if (drawingData.type === 'circle') {
                        shapeElement = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
                        shapeElement.setAttribute('cx', drawingData.cx);
                        shapeElement.setAttribute('cy', drawingData.cy);
                        shapeElement.setAttribute('r', drawingData.r);
                        shapeElement.style.stroke = drawingData.stroke;
                        shapeElement.style.strokeWidth = drawingData.strokeWidth;
                        shapeElement.style.fill = drawingData.fill || 'none';
                        clickElement = shapeElement;
                    ...
                    if (shapeElement) {
                        ...
                        if (clickElement && clickElement !== shapeElement) {
                            clickElement.style.pointerEvents = 'auto';
                            clickElement.style.cursor = 'move';
                            g.appendChild(clickElement);
                        }
                        
                        const targetForEvents = clickElement || shapeElement;
                        
                        // Agregar eventos unificados
                        targetForEvents.addEventListener('mousedown', (e) => {
                            if (this.currentTool === 'eraser') {
                                e.stopPropagation();
                                this.removeDrawing(g.id);
                            } else if (this.currentTool === 'select') {
    ...
    // Función para eliminar un dibujo específico (unificado)
    removeDrawing(id) {
        const element = document.getElementById(id);
        if (element) {
            element.remove();
    ...
    // El resaltado y otras funciones siguen aquí...
